...
std::string getFileNameWithoutExtension(const fs::path &filePath) {
  return filePath.stem().string(); // stem() returns the filename without the extension
}

int convertStringToInt(const std::string &str) {
  try {
    return std::stoi(str); // stoi() converts the string to an integer
...
std::vector<int> processWavFiles(const fs::path &directoryPath) {
  std::vector<int> pitches;
  for (const auto &entry : fs::directory_iterator(directoryPath)) {
    if (entry.is_regular_file() && entry.path().extension() == ".wav") {
      std::string fileNameWithoutExtension = getFileNameWithoutExtension(entry.path());
      if (int fileNameAsInt = convertStringToInt(fileNameWithoutExtension); fileNameAsInt != -1) {
        std::cout << "Filename: " << fileNameWithoutExtension << ", as int: " << fileNameAsInt << std::endl;
        pitches.push_back(fileNameAsInt);
      }
    }
  }
  return pitches;
...
void addCustomPatches() {
  const fs::path path = "./timbre/Custom";
  int i = 38;
  for (const auto &entry : fs::directory_iterator(path)) {
    if (entry.is_directory()) {
      if (auto pitches = processWavFiles(entry.path()); !pitches.empty()) {
        if (i == 38) { // Add a separator before the first custom patch
          SOUND_BANK.push_back(new PatchListSeparator("CUSTOM"));
        }
        SOUND_BANK.push_back(new Timbre(i, "Custom", entry.path().filename().string(), pitches));
        i++;
      }
    }
  }
